        ...
        match write {
            ...
            WriteClause::Merge {
                ...
                let labels = node
                    ...
                    .unwrap_or_default();
                let record =
                    rustyred_core::NodeRecord::new(id.to_string(), labels, Value::Object(props));
                batch
                    .mutations
                    .push(rustyred_core::GraphMutation::NodeUpsert(record));
            }
            WriteClause::Set {
                binding,
                key,
                ...
                for mut node in nodes {
                    ...
                    batch
                        .mutations
                        .push(rustyred_core::GraphMutation::NodeUpsert(node));
                }
            }
            WriteClause::Delete { binding, detach } => {
                ...
                for node in nodes {
                        ...
                        for direction in [Direction::Out, Direction::In] {
                            ...
                            for hit in hits {
                                if let Some(edge) = store.get_edge(&hit.edge_id)? {
                                    ...
                                    batch
                                        .mutations
                                        .push(rustyred_core::GraphMutation::EdgeUpsert(tombstoned));
                                }
                            }
                        }
                    }
                    ...
                    batch
                        .mutations
                        .push(rustyred_core::GraphMutation::NodeUpsert(tombstoned));
                }
            }
        }
    }
