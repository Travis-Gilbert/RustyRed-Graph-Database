...
- Auth required by default. `RUSTY_RED_API_TOKENS` is pre-filled with a
  ...
  `graph:read|graph:write|context:read|admin:read|federation:write`.
...
2. Railway opens the template page. Confirm the listed variables. The
   ...
   `<64-hex-chars>=graph:read|graph:write|context:read|admin:read|federation:write`.
...
- `/`, `/search`, `/search.json`, `/crawl`, `/federate/submit`,
  `/v1/*`, `/mcp`, and `/metrics` reject unauthenticated requests.
