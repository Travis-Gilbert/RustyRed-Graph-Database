...
def test_import_push_ppr():
    from rusty_red_native import push_ppr
...
def test_chain_graph_matches_python_reference():
    ...
    With alpha=0.15, epsilon=1e-4 the algorithm should capture mass on
    every node. Node 1 dominates; node 3 is the most distant.
    """
    from rusty_red_native import push_ppr
    ...
    # Reference values from the Python reference on the same input.
    expected = {
        0: 0.302171130,
        1: 0.358084514,
        2: 0.238209537,
...
def test_non_contiguous_ids_work():
    """Application IDs can be arbitrary ints, e.g. 142857 or 9000001."""
    from rusty_red_native import push_ppr
    adj = {
        142857: [(9000001, 0.7)],
        9000001: [(142857, 0.7)],
